...
public class ShatterableObject {
	...
	public ShatterableObject(GameObject g) {
		...
		//TODO: set mesh tiling here
		Vector2 matScale = mainObject.GetComponent<Renderer>().sharedMaterial.mainTextureScale;
		mainBody = mainObject.GetComponent<Rigidbody>();
	}
	...
	public void setPhysicsEnabled(bool enabled) {
		if (mainBody != null) {
			if (enabled) {
				...
				mainBody.velocity = initialVelocity;
			} else {
				initialPosition = mainObject.transform.position;
				initialRotation = mainObject.transform.rotation;
				initialVelocity = mainBody.velocity;
				mainBody.constraints = RigidbodyConstraints.FreezePositionX | RigidbodyConstraints.FreezePositionY | RigidbodyConstraints.FreezePositionZ | RigidbodyConstraints.FreezeRotationX | RigidbodyConstraints.FreezeRotationY | RigidbodyConstraints.FreezeRotationZ;
			}
		}
	}
