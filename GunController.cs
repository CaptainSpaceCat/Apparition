...
public Vector3 gravity = new Vector3(0f, -9.81f, 0f);
    ...
    void FixedUpdate() {
		...
		RaycastHit hit = getGunArc(transform.position, transform.forward, 12f, gravity);
			if (Input.GetMouseButtonUp (0)) {
                Vector3 iPos = GetComponentInParent<Transform>().position;
                Vector3 offset = GetComponentInParent<PlayerController>().getOffset(hit.normal, playerHeight);
                Vector3 fPos = hit.point + offset;
                
                Vector3 lookDir = Vector3.Normalize(iPos - fPos);
                //fPos = hit.point + GetComponentInParent<PlayerController>().getOffset(lookDir, playerHeight);
                StartCoroutine(GetComponentInParent<PlayerController>().Move(fPos, lookDir, timePeriod));
            ...
            transform.rotation = Quaternion.Euler(yDir, xDir, 0.0f);
        }
    }
